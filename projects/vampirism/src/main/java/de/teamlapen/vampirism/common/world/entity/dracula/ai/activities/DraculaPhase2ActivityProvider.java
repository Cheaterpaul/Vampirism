...
public class DraculaPhase2ActivityProvider extends AiActivityProvider<Dracula> {
    ...
    public Set<SensorType<? extends Sensor<? super Dracula>>> getSensors() {
        return (Set<SensorType<? extends Sensor<? super Dracula>>>) Phase2Activities.SENSORS;
    }

    @Override
    public Set<MemoryModuleType<?>> getMemoryModules() {
        return (Set<MemoryModuleType<?>>) Phase2Activities.MEMORY_MODULES;
    }
    ...
    public Stream<Activity> getActiveActivities() {
        return Stream.concat(Phase2Activities.getActivities(), Stream.of(ModActivities.DRACULA_PHASE_2.get()));
    }
}
